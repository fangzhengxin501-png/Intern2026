...
CSV.write("data_POMCPOW/RS15_30depth&offset_pomcpow_$(timestamp)_$(mean(results)).csv", df)

# 均值和标准差单独存一个 CSV
summary_df = DataFrame(mean_return = mean(results), std_return = std(results))
CSV.write("data_POMCPOW/RS15_30depth&offset_pomcpow_$(timestamp)_summary.csv", summary_df)
