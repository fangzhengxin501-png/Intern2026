...
# 本文件由 test_RS15_ARDESPOT.jl 派生: 除了 RockSamplePOMDP(11, 11) 之外,
...
# 程序结构与 test_POMCPOW/test_RS11_POMCPOW.jl 完全一致
...
# ---- 上下界变体 B [已注释]: 无信息上下界, 与 test_Ada/test_RS11_Ada_v2.jl 的变体 B 一一对应 ----
...
CSV.write("data_ARDESPOT/RS11_K(500)_ardespot_$(timestamp)_$(mean(results))_R1.csv", df)

# 均值和标准差单独存一个 CSV
summary_df = DataFrame(mean_return = mean(results), std_return = std(results))
CSV.write("data_ARDESPOT/RS11_K(500)_ardespot_$(timestamp)_summary_R1.csv", summary_df)
