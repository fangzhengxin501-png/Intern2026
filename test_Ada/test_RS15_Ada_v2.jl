...
# 启用变体 B 时, 必须同时在 test_ARDESPOT/test_RS15_ARDESPOT.jl 里切到同名变体 B,
...
CSV.write("data_Ada/RS15_delta(0.3)_adaops_$(timestamp)_$(mean(results))_R1.csv", df)

# 均值和标准差单独存一个 CSV
summary_df = DataFrame(mean_return = mean(results), std_return = std(results))
CSV.write("data_Ada/RS15_delta(0.3)_adaops_$(timestamp)_summary_R1.csv", summary_df)
